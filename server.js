...
async function iniciarDB() {
  ...
  `);
  await pool.query('ALTER TABLE pedidos ADD COLUMN IF NOT EXISTS notas TEXT;');
...
async function startServer() {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (method === 'POST' && pathname === '/api/pedidos') {
      ...
      const result = await pool.query(
        'INSERT INTO pedidos (numero,cliente,notas,items,total,hora,fecha) VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *',
        [b.numero, b.cliente || null, b.notas || null, JSON.stringify(b.items), b.total, b.hora, hoy()]
      );
    ...
    if (method === 'PUT' && pathname.startsWith('/api/pedidos/')) {
      ...
      const notas     = b.notas     !== undefined ? b.notas     : actual.notas;
      const result = await pool.query(
        'UPDATE pedidos SET items=$1,total=$2,entregado=$3,cliente=$4,notas=$5 WHERE id=$6 RETURNING *',
        [items, total, entregado, cliente, notas, id]
      );
    ...
    if (method === 'GET' && pathname === '/api/export') {
      ...
      let csv = 'ID,Número,Cliente,Notas,Items,Total,Hora,Fecha,Entregado\n';
      result.rows.forEach(r => {
        const items = (Array.isArray(r.items) ? r.items : JSON.parse(r.items))
          .map(i => `${i.qty}x ${i.nombre}`).join(' | ');
        csv += `${r.id},${r.numero},"${r.cliente||''}","${r.notas||''}","${items}",${r.total},${r.hora},${r.fecha},${r.entregado?'Sí':'No'}\n`;
      });
      res.writeHead(200, {
